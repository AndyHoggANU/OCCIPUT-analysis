...
\item The forced component of the variance is significantly correlated with the local wind stress input.
\item The eddy field lags variations in wind stress, with two clear timescales emerging: one at $\sim$6 months, which we attribute to baroclinic instability, and one at 2-3 years, which we attribute to the delayed effect of topographic steering.
\end{keypoints}
...
For example, the eddy saturation hypothesis \citep{Hallberg2006, Meredith2006, Munday2013, Constantinou2019} suggests that the role of eddies in facilitating vertical momentum transport acts to limit the response of ACC transport to changing winds.
...
\citet{Meredith2006} found significant variations in the area-averaged eddy field in some regions, and argued for a 2-3 year lag between events in wind stress forcing and EKE anomalies.
...
\andycomment{Thierry could add more if needed?}
...
It is notable that $E_i$ contains a large component of seasonal variation (Mart\`inez-Moreno et al. 2021, submitted) which dominates the statistics.
...
Thus, we look at the correlation coefficient, $r$, at a range of lags between the EKE and the wind stress $\tau$ (which is identical for all ensemble members).
...
\caption{(a) Snapshot (5-day average) from a single ensemble member showing Southern Ocean EKE, $E_i$, and (b) tome-averagedensemble mean EKE, $\overline{\langle E_i\rangle}$.  }
\label{Fig:1}
\end{center}
\end{figure}
...
The 50-member ensemble of eddy-permitting ocean-sea ice model simulations investigated here demonstrate that inference about the EKE response of a single ensemble member to variable forcing is not robust, consistent with the inference of \citet{Zhang2021}.
...
This heterogeneity, consistent with \citet{Patara2016}, acts to emphasise the differing flow regimes which are found in different parts of the Southern Ocean.
...
The slower timescale is similar to that proposed by \citet{Meredith2006}, based on the single large Southern Ocean wind event in 1999.
 This timescale is consistent with the  topographic feedback mechanism of \citet{Hogg2006}, in which a delayed amplification of the EKE response can occur through the topographic steering of currents away from the zonal direction, which decreases the stability of the current.
...
Given that eddies are critical for the Southern Ocean circulation, this result implies that predictability of the future Southern Ocean will be more difficult to realise than previously thought.
...
AMH is gratefully to the support of 
